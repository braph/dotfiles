...
cmd_diff() {
  ...
  if [ -e "$DEST_DIR/.config/openbox/rc.xml" ]; then
    ...
    echo "No such file or directory: '$DEST_DIR/.config/openbox/rc.xml'"
  fi
  if [ -e "$DEST_DIR/.config/openbox/menu.xml" ]; then
    if ! $DIFF "$BUILD_DIR/.config/openbox/menu.xml" "$DEST_DIR/.config/openbox/menu.xml"; then
