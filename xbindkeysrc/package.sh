...
cmd_build() {
  ...
  pre_build
  mkdir -p "$BUILD_DIR/"
  cp -p "xbindkeysrc" "$BUILD_DIR/.xbindkeysrc"
  post_build
}
...
cmd_install() {
  ...
  mkdir -p "$DEST_DIR/"
  cp -p "$BUILD_DIR/.xbindkeysrc" "$DEST_DIR/.xbindkeysrc"
  post_install
}
...
cmd_diff() {
  ...
  if [ -e "$DEST_DIR/.xbindkeysrc" ]; then
    if ! $DIFF "$BUILD_DIR/.xbindkeysrc" "$DEST_DIR/.xbindkeysrc"; then
      echo " ^--- .xbindkeysrc"
    fi
  else
    echo "No such file or directory: '$DEST_DIR/.xbindkeysrc'"
  fi
}
