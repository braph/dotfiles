...
cmd_build() {
  ...
  cp -p "redshift.conf" "$BUILD_DIR/.config/redshift.conf"
  post_build
}
...
cmd_install() {
  ...
  cp -p "$BUILD_DIR/.config/redshift.conf" "$DEST_DIR/.config/redshift.conf"
  post_install
}
...
cmd_diff() {
  ...
  if [ -e "$DEST_DIR/.config/redshift.conf" ]; then
    if ! $DIFF "$BUILD_DIR/.config/redshift.conf" "$DEST_DIR/.config/redshift.conf"; then
      echo " ^--- .config/redshift.conf"
    fi
  else
    echo "No such file or directory: '$DEST_DIR/.config/redshift.conf'"
  fi
}
