...
cmd_build() {
  ...
  pre_build
  mkdir -p "$BUILD_DIR/.config/netsurf"
  cp -p "Choices" "$BUILD_DIR/.config/netsurf/Choices"
  post_build
}
...
cmd_install() {
  ...
  mkdir -p "$DEST_DIR/.config/netsurf"
  cp -p "$BUILD_DIR/.config/netsurf/Choices" "$DEST_DIR/.config/netsurf/Choices"
  post_install
}
...
cmd_diff() {
  ...
  if [ -e "$DEST_DIR/.config/netsurf/Choices" ]; then
    if ! $DIFF "$BUILD_DIR/.config/netsurf/Choices" "$DEST_DIR/.config/netsurf/Choices"; then
      echo " ^--- .config/netsurf/Choices"
    fi
  else
    echo "No such file or directory: '$DEST_DIR/.config/netsurf/Choices'"
  fi
}
