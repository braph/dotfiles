...
cmd_install() {
  ...
  mkdir -p "$DEST_DIR/"
  cp -p "$BUILD_DIR/.wgetrc" "$DEST_DIR/.wgetrc"
  post_install
}
...
cmd_diff() {
  ...
  if [ -e "$DEST_DIR/.wgetrc" ]; then
    if ! $DIFF "$BUILD_DIR/.wgetrc" "$DEST_DIR/.wgetrc"; then
      echo " ^--- .wgetrc"
    fi
  else
    echo "No such file or directory: '$DEST_DIR/.wgetrc'"
  fi
}
