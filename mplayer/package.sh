...
cmd_build() {
  ...
  cp -p "config" "$BUILD_DIR/.mplayer/config"
  post_build
}
...
cmd_install() {
  ...
  mkdir -p "$DEST_DIR/.mplayer"
  cp -p "$BUILD_DIR/.mplayer/config" "$DEST_DIR/.mplayer/config"
  post_install
}
...
cmd_diff() {
  ...
  if [ -e "$DEST_DIR/.mplayer/config" ]; then
    if ! $DIFF "$BUILD_DIR/.mplayer/config" "$DEST_DIR/.mplayer/config"; then
      echo " ^--- .mplayer/config"
    fi
  else
    echo "No such file or directory: '$DEST_DIR/.mplayer/config'"
  fi
}
