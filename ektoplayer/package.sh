...
cmd_build() {
  ...
  pre_build
  mkdir -p "$BUILD_DIR/.config/ektoplayer"
  cp -p "ektoplayer.rc" "$BUILD_DIR/.config/ektoplayer/ektoplayer.rc"
  post_build
}
...
cmd_install() {
  ...
  mkdir -p "$DEST_DIR/.config/ektoplayer"
  cp -p "$BUILD_DIR/.config/ektoplayer/ektoplayer.rc" "$DEST_DIR/.config/ektoplayer/ektoplayer.rc"
  post_install
}
...
cmd_diff() {
  ...
  if [ -e "$DEST_DIR/.config/ektoplayer/ektoplayer.rc" ]; then
    if ! $DIFF "$BUILD_DIR/.config/ektoplayer/ektoplayer.rc" "$DEST_DIR/.config/ektoplayer/ektoplayer.rc"; then
      echo " ^--- .config/ektoplayer/ektoplayer.rc"
    fi
  else
    echo "No such file or directory: '$DEST_DIR/.config/ektoplayer/ektoplayer.rc'"
  fi
}
