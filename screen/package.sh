...
cmd_build() {
  ...
  pre_build
  mkdir -p "$BUILD_DIR/"
  cp -p "screenrc" "$BUILD_DIR/.screenrc"
  post_build
}
...
cmd_install() {
  ...
  mkdir -p "$DEST_DIR/"
  cp -p "$BUILD_DIR/.screenrc" "$DEST_DIR/.screenrc"
  post_install
}
...
cmd_diff() {
  ...
  if [ -e "$DEST_DIR/.screenrc" ]; then
    if ! $DIFF "$BUILD_DIR/.screenrc" "$DEST_DIR/.screenrc"; then
      echo " ^--- .screenrc"
    fi
  else
    echo "No such file or directory: '$DEST_DIR/.screenrc'"
  fi
}
