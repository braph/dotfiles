...
cmd_build() {
  ...
  pre_build
  mkdir -p "$BUILD_DIR/.config/dunst"
  cp -p "dunstrc" "$BUILD_DIR/.config/dunst/dunstrc"
  post_build
}
...
cmd_install() {
  ...
  mkdir -p "$DEST_DIR/.config/dunst"
  cp -p "$BUILD_DIR/.config/dunst/dunstrc" "$DEST_DIR/.config/dunst/dunstrc"
  post_install
}
...
cmd_diff() {
  ...
  if [ -e "$DEST_DIR/.config/dunst/dunstrc" ]; then
    if ! $DIFF "$BUILD_DIR/.config/dunst/dunstrc" "$DEST_DIR/.config/dunst/dunstrc"; then
      echo " ^--- .config/dunst/dunstrc"
    fi
  else
    echo "No such file or directory: '$DEST_DIR/.config/dunst/dunstrc'"
  fi
}
