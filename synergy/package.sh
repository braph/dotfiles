...
cmd_build() {
  ...
  pre_build
  mkdir -p "$BUILD_DIR/"
  cp -p "synergy.conf" "$BUILD_DIR/.synergy.conf"
  post_build
}
...
cmd_install() {
  ...
  mkdir -p "$DEST_DIR/"
  cp -p "$BUILD_DIR/.synergy.conf" "$DEST_DIR/.synergy.conf"
  post_install
}
...
cmd_diff() {
  ...
  if [ -e "$DEST_DIR/.synergy.conf" ]; then
    if ! $DIFF "$BUILD_DIR/.synergy.conf" "$DEST_DIR/.synergy.conf"; then
      echo " ^--- .synergy.conf"
    fi
  else
    echo "No such file or directory: '$DEST_DIR/.synergy.conf'"
  fi
}
