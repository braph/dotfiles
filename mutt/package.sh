...
HEADER_CACHE_default=header_cache
export HEADER_CACHE="${HEADER_CACHE:-$HEADER_CACHE_default}"

REALNAME_default='Benjamin Abendroth'
export REALNAME="${REALNAME:-$REALNAME_default}"

EMAIL_default=braph93@gmx.de
export EMAIL="${EMAIL:-$EMAIL_default}"
...
cmd_build() {
  ...
  pre_build
  mkdir -p "$BUILD_DIR/.mutt"
  "$FILEPP" -DHEADER_CACHE="$HEADER_CACHE" -DREALNAME="$REALNAME" -DEMAIL="$EMAIL" -M.filepp_modules -m testfile.pm -kc '#>' -ec ENVIRONMENT. muttrc -o "$BUILD_DIR/.mutt/muttrc"
  post_build
}

post_install() {
  mkdir -p "$DEST_DIR/.mutt/$HEADER_CACHE"
}
...
cmd_install() {
  ...
  mkdir -p "$DEST_DIR/.mutt"
  cp -p "$BUILD_DIR/.mutt/muttrc" "$DEST_DIR/.mutt/muttrc"
  post_install
}
...
cmd_diff() {
  ...
  if [ -e "$DEST_DIR/.mutt/muttrc" ]; then
    if ! $DIFF "$BUILD_DIR/.mutt/muttrc" "$DEST_DIR/.mutt/muttrc"; then
      echo " ^--- .mutt/muttrc"
    fi
  else
    echo "No such file or directory: '$DEST_DIR/.mutt/muttrc'"
  fi
}
