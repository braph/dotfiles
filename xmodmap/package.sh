...
KEYBOARD_BASE_LAYOUT_default=us
export KEYBOARD_BASE_LAYOUT="${KEYBOARD_BASE_LAYOUT:-$KEYBOARD_BASE_LAYOUT_default}"

REMAP_NUMPAD_default=1
export REMAP_NUMPAD="${REMAP_NUMPAD:-$REMAP_NUMPAD_default}"

GERMANIZE_default=1
export GERMANIZE="${GERMANIZE:-$GERMANIZE_default}"
...
cmd_build() {
  ...
  pre_build
  mkdir -p "$BUILD_DIR/"
  "$FILEPP" -DKEYBOARD_BASE_LAYOUT="$KEYBOARD_BASE_LAYOUT" -DREMAP_NUMPAD="$REMAP_NUMPAD" -DGERMANIZE="$GERMANIZE" -M.filepp_modules -kc '!>' -ec ENVIRONMENT. Xmodmap -o "$BUILD_DIR/.Xmodmap"
  post_build
}
...
cmd_install() {
  ...
  mkdir -p "$DEST_DIR/"
  cp -p "$BUILD_DIR/.Xmodmap" "$DEST_DIR/.Xmodmap"
  post_install
}

cmd_apply() {
  echo "Applying \"$PACKAGE\" ..." >&2
  setxkbmap $KEYBOARD_BASE_LAYOUT xmodmap ~/.Xmodmap
}
...
cmd_diff() {
  ...
  if [ -e "$DEST_DIR/.Xmodmap" ]; then
    if ! $DIFF "$BUILD_DIR/.Xmodmap" "$DEST_DIR/.Xmodmap"; then
      echo " ^--- .Xmodmap"
    fi
  else
    echo "No such file or directory: '$DEST_DIR/.Xmodmap'"
  fi
}

cmd_show() {
  echo "Showing \"$PACKAGE\" ..." >&2
  printf "export %s=%q\n" KEYBOARD_BASE_LAYOUT "$KEYBOARD_BASE_LAYOUT"
  printf "export %s=%q\n" REMAP_NUMPAD "$REMAP_NUMPAD"
  printf "export %s=%q\n" GERMANIZE "$GERMANIZE"
}
