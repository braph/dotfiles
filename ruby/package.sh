...
cmd_install() {
  ...
  mkdir -p "$DEST_DIR/"
  cp -p "$BUILD_DIR/.irbrc" "$DEST_DIR/.irbrc"
  post_install
}
...
cmd_diff() {
  ...
  if [ -e "$DEST_DIR/.irbrc" ]; then
    if ! $DIFF "$BUILD_DIR/.irbrc" "$DEST_DIR/.irbrc"; then
      echo " ^--- .irbrc"
    fi
  else
    echo "No such file or directory: '$DEST_DIR/.irbrc'"
  fi
}
