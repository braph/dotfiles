...
cmd_build() {
  ...
  pre_build
  mkdir -p "$BUILD_DIR/.elinks"
  cp -p "elinks.conf" "$BUILD_DIR/.elinks/elinks.conf"
  post_build
}
...
cmd_install() {
  ...
  mkdir -p "$DEST_DIR/.elinks"
  cp -p "$BUILD_DIR/.elinks/elinks.conf" "$DEST_DIR/.elinks/elinks.conf"
  post_install
}
...
cmd_diff() {
  ...
  if [ -e "$DEST_DIR/.elinks/elinks.conf" ]; then
    if ! $DIFF "$BUILD_DIR/.elinks/elinks.conf" "$DEST_DIR/.elinks/elinks.conf"; then
      echo " ^--- .elinks/elinks.conf"
    fi
  else
    echo "No such file or directory: '$DEST_DIR/.elinks/elinks.conf'"
  fi
}
