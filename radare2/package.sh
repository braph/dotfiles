...
cmd_build() {
  ...
  pre_build
  mkdir -p "$BUILD_DIR/"
  cp -p "radare2rc" "$BUILD_DIR/.radare2rc"
  post_build
}
...
cmd_install() {
  ...
  mkdir -p "$DEST_DIR/"
  cp -p "$BUILD_DIR/.radare2rc" "$DEST_DIR/.radare2rc"
  post_install
}
...
cmd_diff() {
  ...
  if [ -e "$DEST_DIR/.radare2rc" ]; then
    if ! $DIFF "$BUILD_DIR/.radare2rc" "$DEST_DIR/.radare2rc"; then
      echo " ^--- .radare2rc"
    fi
  else
    echo "No such file or directory: '$DEST_DIR/.radare2rc'"
  fi
}
