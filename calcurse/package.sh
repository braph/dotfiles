...
cmd_build() {
  ...
  pre_build
  mkdir -p "$BUILD_DIR/.calcurse"
  cp -p "keys" "$BUILD_DIR/.calcurse/keys"
  cp -p "conf" "$BUILD_DIR/.calcurse/conf"
  post_build
}
...
cmd_install() {
  ...
  mkdir -p "$DEST_DIR/.calcurse"
  cp -p "$BUILD_DIR/.calcurse/keys" "$DEST_DIR/.calcurse/keys"
  cp -p "$BUILD_DIR/.calcurse/conf" "$DEST_DIR/.calcurse/conf"
  post_install
}
...
cmd_diff() {
  ...
  if [ -e "$DEST_DIR/.calcurse/keys" ]; then
    if ! $DIFF "$BUILD_DIR/.calcurse/keys" "$DEST_DIR/.calcurse/keys"; then
      echo " ^--- .calcurse/keys"
    fi
  else
    echo "No such file or directory: '$DEST_DIR/.calcurse/keys'"
  fi
  if [ -e "$DEST_DIR/.calcurse/conf" ]; then
    if ! $DIFF "$BUILD_DIR/.calcurse/conf" "$DEST_DIR/.calcurse/conf"; then
      echo " ^--- .calcurse/conf"
    fi
  else
    echo "No such file or directory: '$DEST_DIR/.calcurse/conf'"
  fi
}
