...
MUSIC_BASE_default=/media/main/music
export MUSIC_BASE="${MUSIC_BASE:-$MUSIC_BASE_default}"
...
cmd_build() {
  ...
  pre_build
  mkdir -p "$BUILD_DIR/.config/mpd"
  "$FILEPP" -DMUSIC_BASE="$MUSIC_BASE" -M.filepp_modules -kc '#>' -ec ENVIRONMENT. mpd.conf.pp -o "$BUILD_DIR/.config/mpd/mpd.conf"
  post_build
}
...
cmd_install() {
  ...
  mkdir -p "$DEST_DIR/.config/mpd"
  cp -p "$BUILD_DIR/.config/mpd/mpd.conf" "$DEST_DIR/.config/mpd/mpd.conf"
  post_install
}
...
cmd_diff() {
  ...
  if [ -e "$DEST_DIR/.config/mpd/mpd.conf" ]; then
    if ! $DIFF "$BUILD_DIR/.config/mpd/mpd.conf" "$DEST_DIR/.config/mpd/mpd.conf"; then
      echo " ^--- .config/mpd/mpd.conf"
    fi
  else
    echo "No such file or directory: '$DEST_DIR/.config/mpd/mpd.conf'"
  fi
}
