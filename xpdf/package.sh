...
cmd_install() {
  ...
  mkdir -p "$DEST_DIR/"
  cp -p "$BUILD_DIR/.xpdfrc" "$DEST_DIR/.xpdfrc"
  post_install
}
...
cmd_diff() {
  ...
  if [ -e "$DEST_DIR/.xpdfrc" ]; then
    if ! $DIFF "$BUILD_DIR/.xpdfrc" "$DEST_DIR/.xpdfrc"; then
      echo " ^--- .xpdfrc"
    fi
  else
    echo "No such file or directory: '$DEST_DIR/.xpdfrc'"
  fi
}
