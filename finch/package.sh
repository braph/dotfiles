...
cmd_install() {
  ...
  mkdir -p "$DEST_DIR/"
  cp -p "$BUILD_DIR/.gntrc" "$DEST_DIR/.gntrc"
  post_install
}
...
cmd_diff() {
  ...
  if [ -e "$DEST_DIR/.gntrc" ]; then
    if ! $DIFF "$BUILD_DIR/.gntrc" "$DEST_DIR/.gntrc"; then
      echo " ^--- .gntrc"
    fi
  else
    echo "No such file or directory: '$DEST_DIR/.gntrc'"
  fi
}
