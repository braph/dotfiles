...
cmd_build() {
  ...
  pre_build
  mkdir -p "$BUILD_DIR/"
  cp -p "inputrc" "$BUILD_DIR/.inputrc"
  post_build
}
...
cmd_install() {
  ...
  mkdir -p "$DEST_DIR/"
  cp -p "$BUILD_DIR/.inputrc" "$DEST_DIR/.inputrc"
  post_install
}
...
cmd_diff() {
  ...
  if [ -e "$DEST_DIR/.inputrc" ]; then
    if ! $DIFF "$BUILD_DIR/.inputrc" "$DEST_DIR/.inputrc"; then
      echo " ^--- .inputrc"
    fi
  else
    echo "No such file or directory: '$DEST_DIR/.inputrc'"
  fi
}
