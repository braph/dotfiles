...
cmd_build() {
  ...
  pre_build
  mkdir -p "$BUILD_DIR/"
  cp -p "dialogrc" "$BUILD_DIR/.dialogrc"
  post_build
}
...
cmd_install() {
  ...
  mkdir -p "$DEST_DIR/"
  cp -p "$BUILD_DIR/.dialogrc" "$DEST_DIR/.dialogrc"
  post_install
}
...
cmd_diff() {
  ...
  if [ -e "$DEST_DIR/.dialogrc" ]; then
    if ! $DIFF "$BUILD_DIR/.dialogrc" "$DEST_DIR/.dialogrc"; then
      echo " ^--- .dialogrc"
    fi
  else
    echo "No such file or directory: '$DEST_DIR/.dialogrc'"
  fi
}
