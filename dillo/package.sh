...
cmd_diff() {
  ...
  if [ -e "$DEST_DIR/.dillo/style.css" ]; then
    if ! $DIFF "$BUILD_DIR/.dillo/style.css" "$DEST_DIR/.dillo/style.css"; then
      echo " ^--- .dillo/style.css"
    fi
  else
    echo "No such file or directory: '$DEST_DIR/.dillo/style.css'"
  fi
  if [ -e "$DEST_DIR/.dillo/keysrc" ]; then
    if ! $DIFF "$BUILD_DIR/.dillo/keysrc" "$DEST_DIR/.dillo/keysrc"; then
      echo " ^--- .dillo/keysrc"
    fi
  else
    echo "No such file or directory: '$DEST_DIR/.dillo/keysrc'"
  fi
}
