...
cmd_build() {
  ...
  cp -p "user-dirs.dirs" "$BUILD_DIR/.config/user-dirs.dirs"
  post_build
}
...
cmd_install() {
  ...
  cp -p "$BUILD_DIR/.config/user-dirs.dirs" "$DEST_DIR/.config/user-dirs.dirs"
  post_install
}
...
cmd_diff() {
  ...
  if [ -e "$DEST_DIR/.config/user-dirs.dirs" ]; then
    if ! $DIFF "$BUILD_DIR/.config/user-dirs.dirs" "$DEST_DIR/.config/user-dirs.dirs"; then
      echo " ^--- .config/user-dirs.dirs"
    fi
  else
    echo "No such file or directory: '$DEST_DIR/.config/user-dirs.dirs'"
  fi
}
