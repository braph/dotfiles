...
WINDOW_MANAGER_default=openbox
export WINDOW_MANAGER="${WINDOW_MANAGER:-$WINDOW_MANAGER_default}"

STARTPAGE_default=blog.fefe.de
export STARTPAGE="${STARTPAGE:-$STARTPAGE_default}"
...
cmd_build() {
  ...
  pre_build
  mkdir -p "$BUILD_DIR/"
  "$FILEPP" -DWINDOW_MANAGER="$WINDOW_MANAGER" -DSTARTPAGE="$STARTPAGE" -M.filepp_modules -kc '#>' -ec ENVIRONMENT. xinitrc.pp -o "$BUILD_DIR/.xinitrc"
  post_build
}
...
cmd_install() {
  ...
  mkdir -p "$DEST_DIR/"
  cp -p "$BUILD_DIR/.xinitrc" "$DEST_DIR/.xinitrc"
  post_install
}
...
cmd_diff() {
  ...
  if [ -e "$DEST_DIR/.xinitrc" ]; then
    if ! $DIFF "$BUILD_DIR/.xinitrc" "$DEST_DIR/.xinitrc"; then
      echo " ^--- .xinitrc"
    fi
  else
    echo "No such file or directory: '$DEST_DIR/.xinitrc'"
  fi
}
