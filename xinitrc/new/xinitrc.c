...
Begin
   //String LogFile;
   //StringFormat(LogFile, "/tmp/xinitrc.%s.%d.log", Env("USER"), Pid());
   //StringFormat(&LogFile, "/dev/null");
   //ErrToFile(LOG_FILE);
   //OutToFile(LOG_FILE);
...
#define has_run(args...) _has_run(args)
   ...
   Foreach(dir, Strings(".adobe", ".macromedia"))
      ...
      CmdOk Exec("rm", "-rf", StrTmp("%s/%s", Env("HOME"), dir));
      CmdOk Exec("mkdir", "-p", StrTmp("%s/%s", USER_TMP_DIR, dir)) &&
      CmdOk Exec("chmod", "0755", StrTmp("%s/%s", USER_TMP_DIR, dir)) &&
      CmdOk Exec("ln", "-s", StrTmp("%s/%s", USER_TMP_DIR, dir), StrTmp(Env("HOME"), dir));
   Done
   // ======================================================
...
#  define STARTPAGE blog.fefe.de
   ...
   String cmd;
   StringFormat(&cmd, "%s %s", terminal_cmd, terminal_init);
   System(cmd);
   //Free(cmd);
