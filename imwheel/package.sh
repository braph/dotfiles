...
SCROLL_BROWSER_LINES_default=3
export SCROLL_BROWSER_LINES="${SCROLL_BROWSER_LINES:-$SCROLL_BROWSER_LINES_default}"

SCROLL_BROWSER_LINES_SHIFT_default=15
export SCROLL_BROWSER_LINES_SHIFT="${SCROLL_BROWSER_LINES_SHIFT:-$SCROLL_BROWSER_LINES_SHIFT_default}"
...
cmd_build() {
  ...
  pre_build
  mkdir -p "$BUILD_DIR/"
  "$FILEPP" -DSCROLL_BROWSER_LINES="$SCROLL_BROWSER_LINES" -DSCROLL_BROWSER_LINES_SHIFT="$SCROLL_BROWSER_LINES_SHIFT" -M.filepp_modules -kc '#>' -ec ENVIRONMENT. imwheelrc.pp -o "$BUILD_DIR/.imwheelrc"
  post_build
}
...
cmd_install() {
  ...
  mkdir -p "$DEST_DIR/"
  cp -p "$BUILD_DIR/.imwheelrc" "$DEST_DIR/.imwheelrc"
  post_install
}
...
cmd_diff() {
  ...
  if [ -e "$DEST_DIR/.imwheelrc" ]; then
    if ! $DIFF "$BUILD_DIR/.imwheelrc" "$DEST_DIR/.imwheelrc"; then
      echo " ^--- .imwheelrc"
    fi
  else
    echo "No such file or directory: '$DEST_DIR/.imwheelrc'"
  fi
}

cmd_show() {
  echo "Showing \"$PACKAGE\" ..." >&2
  printf "export %s=%q\n" SCROLL_BROWSER_LINES "$SCROLL_BROWSER_LINES"
  printf "export %s=%q\n" SCROLL_BROWSER_LINES_SHIFT "$SCROLL_BROWSER_LINES_SHIFT"
}
