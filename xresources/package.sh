...
COLOR_THEME_default=tango
export COLOR_THEME="${COLOR_THEME:-$COLOR_THEME_default}"

FONTSIZE_default=12
export FONTSIZE="${FONTSIZE:-$FONTSIZE_default}"
...
cmd_build() {
  ...
  pre_build
  mkdir -p "$BUILD_DIR/"
  "$FILEPP" -DCOLOR_THEME="$COLOR_THEME" -DFONTSIZE="$FONTSIZE" -M.filepp_modules -m foreach.pm -Ithemes -kc '#>' -ec ENVIRONMENT. Xresources.pp -o "$BUILD_DIR/.Xresources"
  post_build
}
...
cmd_install() {
  ...
  mkdir -p "$DEST_DIR/"
  cp -p "$BUILD_DIR/.Xresources" "$DEST_DIR/.Xresources"
  post_install
}
...
cmd_diff() {
  ...
  if [ -e "$DEST_DIR/.Xresources" ]; then
    if ! $DIFF "$BUILD_DIR/.Xresources" "$DEST_DIR/.Xresources"; then
      echo " ^--- .Xresources"
    fi
  else
    echo "No such file or directory: '$DEST_DIR/.Xresources'"
  fi
}
