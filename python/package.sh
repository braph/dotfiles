...
cmd_build() {
  ...
  pre_build
  mkdir -p "$BUILD_DIR/"
  cp -p "pythonrc" "$BUILD_DIR/.pythonrc"
  post_build
}
...
cmd_install() {
  ...
  mkdir -p "$DEST_DIR/"
  cp -p "$BUILD_DIR/.pythonrc" "$DEST_DIR/.pythonrc"
  post_install
}
...
cmd_diff() {
  ...
  if [ -e "$DEST_DIR/.pythonrc" ]; then
    if ! $DIFF "$BUILD_DIR/.pythonrc" "$DEST_DIR/.pythonrc"; then
      echo " ^--- .pythonrc"
    fi
  else
    echo "No such file or directory: '$DEST_DIR/.pythonrc'"
  fi
}
