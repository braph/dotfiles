...
THEME_BACKGROUND_default=default
export THEME_BACKGROUND="${THEME_BACKGROUND:-$THEME_BACKGROUND_default}"
...
cmd_build() {
  ...
  pre_build
  mkdir -p "$BUILD_DIR/.aumix"
  "$FILEPP" -DTHEME_COLOR="$THEME_COLOR" -DTHEME_BACKGROUND="$THEME_BACKGROUND" -M.filepp_modules -kc '#>' -ec ENVIRONMENT. color.pp -o "$BUILD_DIR/.aumix/color"
  post_build
}
...
cmd_install() {
  ...
  mkdir -p "$DEST_DIR/.aumix"
  cp -p "$BUILD_DIR/.aumix/color" "$DEST_DIR/.aumix/color"
  post_install
}
...
cmd_diff() {
  ...
  if [ -e "$DEST_DIR/.aumix/color" ]; then
    if ! $DIFF "$BUILD_DIR/.aumix/color" "$DEST_DIR/.aumix/color"; then
      echo " ^--- .aumix/color"
    fi
  else
    echo "No such file or directory: '$DEST_DIR/.aumix/color'"
  fi
}
