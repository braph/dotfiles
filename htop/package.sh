...
cmd_build() {
  ...
  pre_build
  mkdir -p "$BUILD_DIR/.config/htop"
  cp -p "htoprc" "$BUILD_DIR/.config/htop/htoprc"
  post_build
}
...
cmd_install() {
  ...
  mkdir -p "$DEST_DIR/.config/htop"
  cp -p "$BUILD_DIR/.config/htop/htoprc" "$DEST_DIR/.config/htop/htoprc"
  post_install
}
...
cmd_diff() {
  ...
  if [ -e "$DEST_DIR/.config/htop/htoprc" ]; then
    if ! $DIFF "$BUILD_DIR/.config/htop/htoprc" "$DEST_DIR/.config/htop/htoprc"; then
      echo " ^--- .config/htop/htoprc"
    fi
  else
    echo "No such file or directory: '$DEST_DIR/.config/htop/htoprc'"
  fi
}
