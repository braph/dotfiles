...
cmd_build() {
  ...
  pre_build
  mkdir -p "$BUILD_DIR/.config/tint2"
  cp -p "tint2rc" "$BUILD_DIR/.config/tint2/tint2rc"
  post_build
}
...
cmd_install() {
  ...
  mkdir -p "$DEST_DIR/.config/tint2"
  cp -p "$BUILD_DIR/.config/tint2/tint2rc" "$DEST_DIR/.config/tint2/tint2rc"
  post_install
}
...
cmd_diff() {
  ...
  if [ -e "$DEST_DIR/.config/tint2/tint2rc" ]; then
    if ! $DIFF "$BUILD_DIR/.config/tint2/tint2rc" "$DEST_DIR/.config/tint2/tint2rc"; then
      echo " ^--- .config/tint2/tint2rc"
    fi
  else
    echo "No such file or directory: '$DEST_DIR/.config/tint2/tint2rc'"
  fi
}
